...
- MQTT publisher algorithm running service. This service was implemented to run the FEPCM jar (the Java algorithm) and to send to the Raspberry Pis the data generation parameters for its stream via Kafka, to generate the data that will be consumed by the algorithm.
- Experiment file cleaner service. This service was implemented to delete all the files generated by the Java algorithm once the experiment executed has finished and to move de experiment's results to the experiments_results folder in the lib directory.
- Experiments results file listing service. This service was implemented to list all the experiments' results stored in the experiments_results folder.
